...
where $\Delta\in\{10^{-5}, 10^{-4}, 10^{-3}, 10^{-2}, 10^{-1}\}$ and $\varepsilon = 10^{-5}$ introduced to systems that exhibit symmetry breaking. The method creates a plot which colors the phase sapce by the $-\log_{10}(\Delta)$ required to introduce change: the darker the color, the higher the sensitivity of phase classification to the inital conditions.