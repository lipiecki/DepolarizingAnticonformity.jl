...
function sensitivitymap(q::Int, Q::Int, type::Symbol)
    data = load(joinpath("DepolarizingAnticonformityResults", "OutputFiles", "sensitivity_q$(q)_Q$(Q)_$(type).jld2"))
    plt = heatmap(data["intervention_strength"], data["probability_outgroup"], data["sensitivity"]', c=cgrad(:Blues, 6, categorical=true), clims=(0, 5))
